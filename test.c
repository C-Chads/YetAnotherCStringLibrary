...
int main(int argc, char** argv){
	...
	char* buf = malloc(10);
	if(!buf)return 1;
	unsigned long long blen = 0; blen = read_until_terminator(stdin, buf, 10, 'B'); fgetc(stdin); //Used to handle erroneous '\n grabs.'
	...
	long long pos = strfind(text, "mighty"); 
	long long pos2 = strfind(text, "Mighty"); //-1 means it could not find it
	long long pos3 = strfind(text, ""); 
	printf("\n\"mighty\" is at %d\n", pos);
	printf("\nwhich is:%s\n", text+pos);
